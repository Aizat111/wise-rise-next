...
import { PreInfoFormPage } from "@/features/information/pages/PreInfoFormPage";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return buildInformationMetadata({
    locale,
    path: INFORMATION_ROUTES.preInfo,
    title: t("pages.preInfo"),
  });
}

export default async function OnBilgilendirmeRoute({ params }: Props) {
  const { locale } = await params;
  setRequestLocale(locale);

  return <PreInfoFormPage />;
}
